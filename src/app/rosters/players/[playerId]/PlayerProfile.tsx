...
export default function PlayerProfile({ member }: { member: TeamMember }) {
  ...
  const primaryCountry = member.countries?.[0];
                ...
                {primaryCountry && (
                  <>
                    <motion.span
                      ...
                      transition={{ duration: 0.5 }}
                    >
                      {getFlagEmoji(primaryCountry.code)}
                    </motion.span>
              ...
              >
                {member.type === "player" ? (
                  ...
                  member.roles?.map((role) => (
                    <div
                      key={role}
                      className="px-3 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r from-[#D4AF37]/20 to-[#FFD700]/20 text-[#FFD700] rounded-xl text-xs sm:text-sm font-bold border border-[#D4AF37]/40 backdrop-blur-sm"
                    >
                      {role}
                    </div>
                  ))
        ...
        <div className="grid lg:grid-cols-3 gap-6 sm:gap-8 max-w-7xl mx-auto lg:items-start">
          <div className="lg:col-span-2 space-y-6 sm:space-y-8">
            ...
            >
              ...
              <p className="text-white/80 text-base sm:text-lg leading-relaxed select-none">
                {member.bio}
              </p>
            </motion.div>
                ...
                <div className="grid sm:grid-cols-2 gap-3 sm:gap-4">
                  {member.achievements.map((achievement, idx) => (
                    ...
                    >
                      ...
                      <span className="text-white/90 text-sm sm:text-base  select-none">
                        {achievement}
                      </span>
                    </motion.div>
                  ))}
                      ...
                      <div className="relative aspect-video rounded-xl overflow-hidden bg-white/5 border border-white/10 group-hover:border-[#D4AF37]/60 transition-all duration-300 shadow-lg group-hover:shadow-[#D4AF37]/20">
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent flex items-end p-3 sm:p-4">
                          <div className="flex items-center justify-between w-full">
                            <span className="text-xs sm:text-sm font-semibold group-hover:text-[#FFD700] transition-colors  select-none">
                              {clip.title}
                            </span>
                            <FiExternalLink
                              className="text-white/60 group-hover:text-[#FFD700] transition-colors shrink-0 ml-2"
                              size={16}
                            />
                          </div>
                        </div>
            ...
            >
              ...
              <div className="space-y-3">
                {member.socials.liquipedia && (
                  <SocialLink
                    href={member.socials.liquipedia}
                    icon={FiBook}
                    label="Liquipedia"
...
function SocialLink({
  ...
  return (
    ...
    >
      ...
      <FiExternalLink
        className="ml-auto opacity-0 group-hover:opacity-100 transition-all shrink-0"
        size={14}
      />
    </motion.a>
  );
