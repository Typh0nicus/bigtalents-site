...
type Tier = {
  ...
  iconBg: string; // bg class e.g. 'bg-blue-500/10'
  iconColor: string; // text class e.g. 'text-blue-400'
  particleColor: string; // rgba string for tiny particles
  featured?: boolean;
};
...
const PLATFORMS: SocialPlatform[] = [
  { icon: FaYoutube, name: "YouTube", colorClass: "text-red-500", stat: "Video Content" },
  { icon: FaTwitch, name: "Twitch", colorClass: "text-purple-500", stat: "Live Streams" },
  { icon: SiTiktok, name: "TikTok", colorClass: "text-white", stat: "Short Form" },
];

// Map a text color class to a background glow class (Tailwind-safe)
...
function PlatformIcon({ platform, index }: { platform: SocialPlatform; index: number }) {
  const controls = useAnimation();
  const prefersReduced = useReducedMotion();
  const glowBg = COLOR_TO_BG[platform.colorClass] ?? "bg-white/20";
  ...
  return (
    <motion.div
      ...
      onHoverStart={() =>
        controls.start(
          prefersReduced ? {} : { y: -8, scale: 1.05, rotate: [0, 5, -5, 0] }
        )
      }
      onHoverEnd={() => controls.start({ y: 0, scale: 1, rotate: 0 })}
      animate={controls}
      className="relative group cursor-pointer"
      aria-label={`${platform.name} — ${platform.stat}`}
    >
      <div className="flex flex-col items-center gap-3 p-6 bg-white/[0.03] rounded-2xl border border-white/10 hover:border-white/30 transition-all duration-300">
        <div className="p-4 bg-white/5 rounded-xl group-hover:bg-white/10 transition-colors duration-300">
          <platform.icon className={cn(platform.colorClass, "text-4xl")} />
        </div>
      ...
      {/* Proper glow using Tailwind classes, not inline style */}
      <motion.div
        className={cn("absolute inset-0 rounded-2xl blur-xl -z-10", glowBg)}
        initial={{ opacity: 0 }}
        whileHover={prefersReduced ? {} : { opacity: 0.5 }}
...
export function CreatorProgram() {
  ...
  // Fewer particles on low-motion devices
  const PARTICLE_POSITIONS = Array.from(
    { length: prefersReduced ? 0 : 14 },
    (_, i) => ({
      ...
      <section className="relative py-32 w-full">
        ...
        <div className="container relative z-10 px-4 w-full mx-auto">
          ...
          >
              ...
              {PERKS.map((perk, i) => (
                ...
                >
                  <div className="inline-flex items-center justify-center w-12 h-12 bg-white/5 rounded-xl mb-4 group-hover:scale-110 transition-transform duration-300">
                    <perk.icon className={cn(perk.color, "text-2xl")} />
                  </div>
            ...
            >
              <motion.div whileHover={isMounted ? { scale: 1.02 } : {}} whileTap={isMounted ? { scale: 0.98 } : {}}>
                <Link
        ...
        <div className="relative max-w-6xl mx-auto w-full">
          <div className="grid lg:grid-cols-3 gap-8 lg:gap-6">
            {TIERS.map((tier, index) => {
              ...
              const tierParticles = Array.from(
                { length: prefersReduced ? 0 : 12 },
                (_, i) => ({
                ...
                >
                  ...
                  >
                    ...
                    <div className="mb-6 relative z-10">
                      <div className={cn("inline-flex p-6 rounded-2xl backdrop-blur-sm relative", tier.iconBg)}>
                        <IconComponent className={cn(tier.iconColor, "text-4xl")} />
                      </div>
                    </div>
                    ...
                    <motion.div
                      className={cn("absolute inset-0 rounded-2xl bg-gradient-to-br pointer-events-none z-0", tier.color)}
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="text-white/70 mb-10 max-w-2xl mx-auto leading-relaxed px-4">
              A majority of content must be <span className="text-[#D4AF37] font-semibold">Brawl Stars related</span>.
            ...
            <div className="relative z-10 w-full">
              ...
              >
                <h2 className="text-3xl sm:text-4xl md:text-5xl font-black mb-6 px-4">
                  Ready to{" "}
                  <span className="bg-gradient-to-r from-[#D4AF37] via-purple-400 to-[#D4AF37] bg-clip-text text-transparent">
                    Level Up?
                  </span>
                </h2>
